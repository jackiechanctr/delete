  ...
  return (
    <nav className="sticky top-4 z-50 mx-auto flex w-full max-w-6xl items-center justify-between rounded-full border border-violet-100 bg-white/95 px-4 py-2 shadow-[0_8px_25px_rgba(124,58,237,0.1)] backdrop-blur">
      <Link href="/" className="flex items-center gap-2">
        <span className="inline-block h-7 w-7 rounded-full bg-violet-100" />
...
/* ══════════════════════════════════════════════════════════════════
   NEW — Partner Logos Infinite Scroll
   ══════════════════════════════════════════════════════════════════ */
function PartnersSection() {
  const logos = ['HDFC', 'Axis', 'ICICI', 'SBI', 'Kotak', 'TATA', 'ADANI', 'RELIANCE', 'INFY', 'TCS'];
    ...
    <section className="mx-auto mt-16 max-w-6xl px-6">
      ...
      >
        <p className="text-[10px] font-bold uppercase tracking-[0.28em] text-slate-400">
          Trusted by Teams Building the Future of Wealth
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        className="relative mt-5 overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]"
      >
        <motion.div
        ...
        >
          {[...logos, ...logos].map((name, i) => (
            <span key={i} className="inline-flex items-center gap-2 text-lg font-black text-slate-200 transition-colors hover:text-violet-400">
...
/* ══════════════════════════════════════════════════════════════════
   NEW — How It Works (5-step with connector line)
   ══════════════════════════════════════════════════════════════════ */
      ...
      <div className="mt-14 grid gap-10 md:grid-cols-2 lg:grid-cols-3">
        {steps.map((step, i) => (
          ...
          >
            ...
            <h3 className="mt-2.5 text-2xl font-black text-slate-900">{step.title}</h3>
            <p className="mt-3 text-lg leading-relaxed text-slate-700">{step.desc}</p>
          </motion.article>
        ))}
      </div>
    </section>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-6 sm:px-6 lg:px-8">
        ...
        <section className="mx-auto mt-24 max-w-6xl px-6">
          <div className="text-center">
            ...
            <h2 className="mt-4 text-3xl font-bold text-slate-900 sm:text-4xl">Everything you need to run modern wealth operations</h2>
            <p className="mt-3 text-slate-600">Purpose-built modules that scale from pilot to enterprise.</p>
          </div>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {[
              {
                title: 'Identity Unification',
                desc: 'Single source of truth across PAN, email & mobile.',
                icon: <Users className="h-5 w-5" />,
                accent: 'from-violet-500 to-indigo-500',
              },
              {
                title: 'Wealth Engine',
                desc: 'Real-time net worth, allocation & performance analytics.',
                icon: <BarChart3 className="h-5 w-5" />,
                accent: 'from-blue-500 to-cyan-500',
              },
              {
                title: 'Orchestration',
                desc: 'Fault-tolerant workflows with retries & circuit breakers.',
                icon: <Building2 className="h-5 w-5" />,
                accent: 'from-emerald-500 to-teal-500',
              },
              {
                title: 'Realtime Alerts',
                desc: 'WebSocket-powered proactive operations monitoring.',
              ...
              <motion.article
                ...
                whileHover={{ y: -6 }}
                className="group relative overflow-hidden rounded-3xl border border-violet-100 bg-white p-6 shadow-sm transition hover:shadow-xl hover:shadow-violet-500/10"
              >
                <div className={`inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br text-white ${cap.accent}`}>
                  {cap.icon}
                </div>
                <h3 className="mt-4 text-lg font-bold text-slate-900">{cap.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">{cap.desc}</p>
                <div className={`mt-5 h-1 w-12 rounded-full bg-gradient-to-r transition-all duration-300 group-hover:w-20 ${cap.accent}`} />
              </motion.article>
            ))}
          </div>
        </section>
