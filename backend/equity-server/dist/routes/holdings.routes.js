...
export async function holdingsRoutes(app) {
    app.addHook('onRequest', app.authenticate);
    app.get('/api/v1/holdings', async (req, reply) => {
        const query = z
            .object({
    ...
    app.get('/api/v1/holdings/:investorRef/summary', async (req, reply) => {
        const { investorRef } = req.params;
        const holdings = await prisma.holding.findMany({ where: { investorRef } });
        if (!holdings.length)
            return reply.status(404).send(fail('No holdings found', 'NOT_FOUND', 404));
