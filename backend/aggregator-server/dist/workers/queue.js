...
export function startWorkers() {
    ...
    new Worker('wealth-sync', async (job) => {
        const { investorId, investorRef } = job.data;
    ...
    new Worker('alerts', async (job) => {
        const { title, message, severity, source } = job.data;
        await prisma.alert.create({ data: { title, message, severity, source } });
    }, { connection });
    logger.info('BullMQ workers started');
}
export async function enqueueSync(investorId, investorRef) {
    if (syncQueue) {
